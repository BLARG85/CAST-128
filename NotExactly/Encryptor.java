...
public class Encryptor implements Cipher_Require
{
	...
	public static void main( String args[] ) throws IOException
	{
		...
		*/
		...
		int int32 = 0b01111111111111111111111111111111;  ///********
		int int322 = 0b10000000000000000000000000000000;
		int test = 0b010101;
		int test1  = 0b11111111111111111111111111101011;
		short int16 = 0b0101010110010101;
	...
	private static final int[] unscramble(int x)
	{
	  ...
	  public Object makeKey(byte[] uk, int bs) throws InvalidKeyException
	  {
	    ...
	    Cast128Key result = new Cast128Key();
	    result.rounds = (len < 11) ? _12_ROUNDS : _16_ROUNDS;
	    byte[] createdKEY = new byte[16];
	    System.arraycopy(uk, 0, createdKEY, 0, len);
	    ...
	    int[]bake;
	    int x0x1x2x3 =  createdKEY[0 ]         << 24
	                 | (createdKEY[1 ] & 0xFF) << 16
	                 | (createdKEY[2 ] & 0xFF) << 8
	                 | (createdKEY[3 ] & 0xFF);
	    int x4x5x6x7 =  createdKEY[4 ]         << 24
	                 | (createdKEY[5 ] & 0xFF) << 16
	                 | (createdKEY[6 ] & 0xFF) << 8
	                 | (createdKEY[7 ] & 0xFF);
	    int x8x9xAxB =  createdKEY[8 ]         << 24
	                 | (createdKEY[9 ] & 0xFF) << 16
	                 | (createdKEY[10] & 0xFF) << 8
	                 | (createdKEY[11] & 0xFF);
	    int xCxDxExF =  createdKEY[12]         << 24
	                 | (createdKEY[13] & 0xFF) << 16
	                 | (createdKEY[14] & 0xFF) << 8
	                 | (createdKEY[15] & 0xFF);
	   bake = unscramble(x0x1x2x3);
	    x0 =bake[0];
	    x1 =bake[1];
	    x2 =bake[2];
	    x3 =bake[3];
	   bake = unscramble(x4x5x6x7);
	    x4 =bake[0];
	    x5 =bake[1];
	    x6 =bake[2];
	    x7 =bake[3];
	   bake = unscramble(x8x9xAxB);
	    x8 =bake[0];
	    x9 =bake[1];
	    xA =bake[2];
	    xB =bake[3];
	   bake = unscramble(xCxDxExF);
	    xC =bake[0];
	    xD =bake[1];
	    xE =bake[2];
	    xF =bake[3];
	    z0z1z2z3 = x0x1x2x3 ^ S5[xD] ^ S6[xF] ^ S7[xC] ^ S8[xE] ^ S7[x8];
	   bake = unscramble(z0z1z2z3);
	    z0 =bake[0];
	    z1 =bake[1];
	    z2 =bake[2];
	    z3 =bake[3];
	    z4z5z6z7 = x8x9xAxB ^ S5[z0] ^ S6[z2] ^ S7[z1] ^ S8[z3] ^ S8[xA];
	   bake = unscramble(z4z5z6z7);
	    z4 =bake[0];
	    z5 =bake[1];
	    z6 =bake[2];
	    z7 =bake[3];
	    z8z9zAzB = xCxDxExF ^ S5[z7] ^ S6[z6] ^ S7[z5] ^ S8[z4] ^ S5[x9];
	   bake = unscramble(z8z9zAzB);
	    z8 =bake[0];
	    z9 =bake[1];
	    zA =bake[2];
	    zB =bake[3];
	    zCzDzEzF = x4x5x6x7 ^ S5[zA] ^ S6[z9] ^ S7[zB] ^ S8[z8] ^ S6[xB];
	   bake = unscramble(zCzDzEzF);
	    zC =bake[0];
	    zD =bake[1];
	    zE =bake[2];
	    zF =bake[3];
	  ...
	  // Used Internet for help
	  //does left and right side as pairs using the keys generated
	  public void encrypt(byte[] in, int i, byte[] out, int j, Object k, int bs)
	  {
	    if (bs != DEFAULT_BLOCK_SIZE)
	      throw new IllegalArgumentException();
	    Cast128Key K = (Cast128Key) k;
	    int L = (in[i++] & 0xFF) << 24
	          | (in[i++] & 0xFF) << 16
	          | (in[i++] & 0xFF) << 8
	          |  in[i++] & 0xFF;
