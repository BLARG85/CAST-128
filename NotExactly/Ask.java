...
public class Ask{
	...
	public Ask(String p) throws IOException, InvalidKeyException
	{
	...
	public Ask(int u) throws IOException, InvalidKeyException
	{
		Encryptor run = new Encryptor();
		...
		**/
		...
		for (int y=0; y<BITS; y+=8){
			byte[] OUTPUT = new byte[8];
			run.decrypt(INPUT, y, OUTPUT, 0, KEY, 8);
			String DECRYPTED = new String(OUTPUT, Charset.defaultCharset());
			for (int d=0; d<8; d++)  DECRYPTED_INFORMATION[d+y] = OUTPUT[d];
